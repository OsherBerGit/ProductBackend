...
צור מחלקת מודל של מוצר (`com.example.demo.entity.Product`) עם האנוטציות הנדרשות:
...
צור ממשק `ProductRepository` שיורש מ- `<JpaRepository<com.example.demo.entity.Product, Long`:
...
- יישם את הפעולות הבאות:
    - `createProduct(com.example.demo.entity.Product product)` - יצירת מוצר חדש
    - `getAllProducts()` - קבלת כל המוצרים
    - `getProductById(Long id)` - קבלת מוצר לפי מזהה
    - `updateProduct(Long id, com.example.demo.entity.Product productDetails)` - עדכון מוצר קיים
...
- יישם את הפעולות הבאות:
    ...
    - `createProduct(@RequestBody com.example.demo.entity.Product product)` - `@PostMapping`
    - `getAllProducts()` - `@GetMapping`
    - `getProductById(@PathVariable Long id)` - `@GetMapping("/{id}")`
    - `updateProduct(@PathVariable Long id, @RequestBody com.example.demo.entity.Product product)` - `@PutMapping("/{id}")`
